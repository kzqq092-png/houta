...
**主要数据库文件：**
- `db/factorweave_system.db` - 系统配置和用户数据
- `db/indicators.db` - 技术指标定义和参数  
- `db/metrics.db` - 性能监控数据
- `db/strategies.db` - 策略定义和回测结果
...
class ConfigManager:
    def __init__(self):
        self.sqlite_conn = sqlite3.connect('db/factorweave_system.db')
...
class HybridDataManager:
    def __init__(self):
        self.sqlite_db = SQLiteDataAccess('db/factorweave_system.db')
