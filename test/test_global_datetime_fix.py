...
def test_module_kdata_preprocess(module_name, preprocess_func, test_data, context="测试"):
    ...
    try:
        # 测试datetime在索引中的数据
        print("  1. 测试datetime在索引中的数据")
        test_data1 = create_test_data_with_datetime_in_index()
        print(f"     原始数据: 索引类型={type(test_data1.index)}, 索引名={test_data1.index.name}")
        print(f"     原始列: {list(test_data1.columns)}")

        result1 = preprocess_func(test_data1, f"{context}1")
        print(f"     处理后: 形状={result1.shape}, 列={list(result1.columns)}")
        ...
        print(f"     原始列: {list(test_data2.columns)}")

        result2 = preprocess_func(test_data2, f"{context}2")
        print(f"     处理后: 形状={result2.shape}, 列={list(result2.columns)}")
...
def main():
    ...
    # 测试5: backtest.backtest_engine
    try:
        from backtest.backtest_engine import StrategyBacktester
        # 创建一个简单的测试数据用于初始化
        simple_data = pd.DataFrame({
            'open': [10], 'high': [15], 'low': [5], 'close': [12], 'volume': [1000]
        })
        backtester = StrategyBacktester(simple_data)
        result = test_module_kdata_preprocess("backtest.backtest_engine", backtester._kdata_preprocess, None, "backtest_engine")
        test_results.append(result)
    except Exception as e:
        print(f"❌ backtest.backtest_engine 导入失败: {e}")
        test_results.append(False)
    ...
    print(f"通过模块数: {passed}")
    print(f"失败模块数: {total - passed}")
    print(f"通过率: {passed/total*100:.1f}%")

    if passed == total:
        print("\n🎉 所有模块的datetime字段处理都已修复！")
        print("✅ 系统现在可以正确处理来自hikyuu和其他数据源的K线数据")
        print("✅ 不再会出现'datetime字段缺失'的错误")
        print("✅ 所有模块使用统一的数据处理逻辑")
    else:
        print(f"\n⚠️  有 {total - passed} 个模块需要进一步检查")
        print("请检查失败的模块并进行修复")
