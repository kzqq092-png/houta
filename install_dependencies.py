...
def install_from_requirements(requirements_file):
    ...
    if not os.path.exists(requirements_file):
        logger.info(f"错误：{requirements_file} 文件不存在！")
        return False
    ...
    try:
        ...
        logger.info("requirements.txt 中的依赖安装成功！")
        return True
    except subprocess.CalledProcessError:
...
def main():
    ...
    logger.info("========== 合适的买卖点交易系统 - 依赖安装 ==========")
    logger.info("将要安装以下依赖：")
    logger.info("1. requirements.txt 中的基础依赖")
    logger.info("2. seaborn (用于可视化)")
    logger.info("3. PyQt5 (用于GUI界面)")
    logger.info("4. PyQtWebEngine (用于GUI中的Web内容)")
