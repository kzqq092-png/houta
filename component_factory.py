...
class ComponentFactory:
    ...
    def create_signal(params=None):
        ...
        def signal_calculate(k, record):
            try:
                ...
                rsi_buy_threshold = params.get('rsi_buy_threshold', 30) if params else 30
                rsi_sell_threshold = params.get('rsi_sell_threshold', 70) if params else 70
    ...
    def create_money_manager(params=None):
        ...
        def get_buy_num(tm, datetime, stk, price, risk, part_from):
            try:
                ...
                # 计算动态仓位比例
                position_ratio = params.get('position_size', 0.2) if params else 0.2
                
                # 计算最大可用资金
                max_cash = total_assets * position_ratio
                
                # 计算每笔交易的风险金额
                risk_amount = total_assets * (params.get('risk_per_trade', 0.02) if params else 0.02)
                
                # 获取ATR值
                k = stk.get_kdata(Query(datetime - 30, datetime))
                if len(k) > 0:
                    df = k.to_df()
                    atr = calculate_atr(df, params.get('atr_period', 14) if params else 14)
                    
                    # 根据ATR计算止损价格
                    stop_loss = price - atr.iloc[-1] * (params.get('atr_multiplier', 2) if params else 2)
                    risk_per_share = price - stop_loss
                    
                    # 计算可买入数量
                    num = int(risk_amount / risk_per_share) if risk_per_share > 0 else 0
        ...
        def stoploss_calculate(datetime, stock, price, risk, part_from):
            try:
                ...
                if len(k) > 0:
                    ...
                    # 计算动态止损价格
                    atr_multiplier = params.get('atr_multiplier', 2) if params else 2
                    
                    # ATR止损
                    dynamic_stop = price - atr.iloc[-1] * atr_multiplier
                    
                    # 固定百分比止损
                    fixed_stop_loss = params.get('fixed_stop_loss', 0.05) if params else 0.05
    ...
    def create_takeprofit(params=None):
        ...
        返回:
            ProfitGoalBase: 止盈组件
        """
        def takeprofit_calculate(datetime, stock, price, risk, part_from):
            try:
                # 获取历史数据
                k = stock.get_kdata(Query(datetime - 30, datetime))
                if len(k) > 0:
                    ...
                    # 计算动态止盈价格
                    atr_multiplier = params.get('atr_multiplier', 3) if params else 3
                    
                    # ATR止盈
                    dynamic_tp = price + atr.iloc[-1] * atr_multiplier
                    
                    # 固定百分比止盈
                    fixed_take_profit = params.get('fixed_take_profit', 0.1) if params else 0.1
                    fixed_tp = price * (1 + fixed_take_profit)
                    
                    # 使用更保守的止盈价格（更容易达到）
                    take_profit_price = min(dynamic_tp, fixed_tp)
                    
                    return take_profit_price
                    
                return price * (1 + (params.get('fixed_take_profit', 0.1) if params else 0.1))
            except Exception as e:
                print(f"止盈计算错误: {str(e)}")
                return price * (1 + (params.get('fixed_take_profit', 0.1) if params else 0.1))
            
        return crtPG(takeprofit_calculate, params=params, name='EnhancedTakeprofit')
    ...
    def create_environment(params=None):
        ...
        返回:
            EnvironmentBase: 环境组件
        """
        def environment_calculate(k):
            # 简单的市场环境判断，这里可以扩展更复杂的逻辑
            return True
            
        return crtEV(environment_calculate, params=params, name='EnhancedEnvironment')
    ...
    def create_condition(params=None):
        ...
        返回:
            ConditionBase: 条件组件
        """
        def condition_calculate(k):
            # 简单的条件判断，这里可以扩展更复杂的逻辑
            return True
            
        return crtCN(condition_calculate, params=params, name='EnhancedCondition')
    ...
    def create_trade_cost(params=None):
        ...
        return TC_FixedA(
            ...
            lowest_commission=params.get('lowest_commission', 5.0) if params else 5.0,
            stamptax=params.get('stamptax', 0.001) if params else 0.001,
            transferfee=params.get('transferfee', 0.001) if params else 0.001,
            lowest_transferfee=params.get('lowest_transferfee', 1.0) if params else 1.0
        )
    ...
    def create_trade_manager(params=None):
        ...
        # 创建交易管理器
        return crtTM(
            init_cash=params.get('init_cash', 100000.0) if params else 100000.0,
            cost_func=tc,
    ...
    def create_system(params=None):
        ...
        tp = ComponentFactory.create_stoploss(params)  # 止盈策略也使用止损组件
