...
def detect_market_regime(df, price_col='close', window=200, threshold=0.1, regime_window=20):
    ...
    # 牛市条件：价格高于长期均线且斜率为正
    bull_condition = (result_df['price_dev'] > threshold) & (result_df['slope'] > 0)
    result_df.loc[bull_condition, 'raw_market_state'] = 1
    
    # 熊市条件：价格低于长期均线且斜率为负
    bear_condition = (result_df['price_dev'] < -threshold) & (result_df['slope'] < 0)
...
def detect_market_volatility(df, price_col='close', window=20, high_vol_percentile=0.8):
    ...
    result_df.loc[result_df['vol_percentile'] > high_vol_percentile, 'high_volatility'] = 1
...
def identify_trend_reversal(df, price_col='close', short_window=20, long_window=50, signal_threshold=0.02):
    ...
    # 计算短期和长期移动平均线
    result_df['short_ma'] = result_df[price_col].rolling(window=short_window).mean()
    result_df['long_ma'] = result_df[price_col].rolling(window=long_window).mean()
    ...
    for i in range(long_window, len(result_df)):
        ...
        rsi_oversold = result_df['rsi'].iloc[i-1] < 30 and result_df['rsi'].iloc[i] > 30
        
        price_higher_low = all(result_df[price_col].iloc[i-3:i] > result_df[price_col].iloc[i-4])
        
        # 看跌反转条件
        bearish_ma_crossover = (result_df['short_ma'].iloc[i-1] >= result_df['long_ma'].iloc[i-1]) and \
                              (result_df['short_ma'].iloc[i] < result_df['long_ma'].iloc[i])
        
        rsi_overbought = result_df['rsi'].iloc[i-1] > 70 and result_df['rsi'].iloc[i] < 70
        
        price_lower_high = all(result_df[price_col].iloc[i-3:i] < result_df[price_col].iloc[i-4])
    ...
    for i in range(1, len(result_df)):
        if result_df['trend_reversal'].iloc[i-1] == 1:
            ...
            if i+5 < len(result_df):
                future_return = result_df[price_col].iloc[i+5] / result_df[price_col].iloc[i] - 1
...
def plot_market_regimes(df, price_col='close', figsize=(15, 10)):
    ...
    if 'ma_long' in df.columns:
        ax1.plot(df.index, df['ma_long'], label='Long MA', color='orange', alpha=0.7)
    
    if 'short_ma' in df.columns and 'long_ma' in df.columns:
        ax1.plot(df.index, df['short_ma'], label='Short MA', color='green', alpha=0.7)
        ax1.plot(df.index, df['long_ma'], label='Long MA', color='red', alpha=0.7)
    ...
    # 绘制市场状态
    ax2.plot(df.index, df['market_state'], color='purple', label='Market State')
    ...
    if 'volatility' in df.columns and 'high_volatility' in df.columns:
        ax3.plot(df.index, df['volatility'], color='orange', label='Volatility')
