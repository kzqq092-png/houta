...
def generate_enhanced_trading_signals(df):
    ...
    if all(col in df.columns for col in ['buy_prob', 'sell_prob']):
        ...
        result_df.loc[result_df['buy_prob'] > buy_threshold, 'signal'] = 1  # 买入信号
        result_df.loc[result_df['sell_prob'] > sell_threshold, 'signal'] = -1  # 卖出信号
    else:
        # 直接使用分类模型的预测
...
def optimize_signal_generation(df):
    ...
    if 'volatility_20d' in result_df.columns:
        ...
        result_df['sell_threshold'] = base_sell_threshold + vol_percentile * 0.2
    else:
        # 固定阈值
    ...
    # 应用优化阈值
    result_df.loc[result_df['buy_prob'] > result_df['buy_threshold'], 'optimized_signal'] = 1
    result_df.loc[result_df['sell_prob'] > result_df['sell_threshold'], 'optimized_signal'] = -1
    
    # 4. 信号冲突处理（买入和卖出信号同时满足条件）
    conflicts = (result_df['buy_prob'] > result_df['buy_threshold']) & (result_df['sell_prob'] > result_df['sell_threshold'])
    if conflicts.any():
        # 选择概率更高的信号
        stronger_buy = result_df.loc[conflicts, 'buy_prob'] > result_df.loc[conflicts, 'sell_prob']
    ...
    if 'position' in result_df.columns and 'close' in result_df.columns:
        ...
        for i in range(1, len(result_df)):
            if result_df['position'].iloc[i-1] == 1:  # 之前是多头
                # 计算价格变动
                price_change = (result_df['close'].iloc[i] / result_df['close'].iloc[i-1]) - 1
