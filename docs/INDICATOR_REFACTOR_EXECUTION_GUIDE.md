...
YS-Quant‌项目的指标系统重构旨在将系统从当前的混合实现，迁移至一个统一、动态、数据库驱动的全新架构。目前已经完成了基础设施建设阶段，包括数据库设计、算法库实现、核心服务和适配器实现。本指南将帮助您完成剩余的重构工作。
...
YS-Quant‌的指标系统采用数据库驱动的架构，支持动态加载和计算指标。
