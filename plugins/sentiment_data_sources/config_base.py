...
class ConfigurablePlugin(ABC):
    ...
    def load_config(self, config_file: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if not is_valid:
                self._safe_log("warning", f"配置验证失败: {error_msg}，使用默认配置")
                self._config = self.get_default_config()
    ...
    def save_config(self, config: Dict[str, Any] = None) -> bool:
        ...
        if config is not None:
            ...
            if not is_valid:
                self._safe_log("error", f"配置验证失败: {error_msg}")
                return False
            self._config = config
        ...
        try:
            ...
            self._safe_log("info", f"配置已保存到: {self._config_file}")
            return True
    ...
    def set_config(self, key: str, value: Any) -> bool:
        ...
        is_valid, error_msg = self.validate_config(self._config)
        if not is_valid:
            self._safe_log("error", f"配置更新失败: {error_msg}")
            return False
