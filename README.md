...
## 系统概述
FactorWeave-Quant 量化交易系统是一个功能完整的量化交易平台，支持策略开发、回测、实盘交易等功能。系统采用Python编写，基于PyQt5构建用户界面，具有良好的可扩展性和易用性。通过模块化设计，系统集成了数据获取、技术分析、策略实现、回测验证和实盘交易等全链路功能。

### 🆕 最新版本特性 (v2.0)
- **🚀 DuckDB专业数据导入**：高性能数据导入引擎，支持多种数据源和格式
- **🧠 AI智能预测服务**：集成机器学习模型，提供执行时间预测和参数优化
- **📊 统一性能监控**：实时监控系统性能，支持多维度指标分析
- **⚡ 多级缓存系统**：内存+磁盘多级缓存，显著提升数据访问性能
- **🔄 增强事件总线**：支持优先级队列、持久化和背压管理
- **🌐 分布式任务执行**：智能负载均衡和故障转移机制
- **🛡️ 增强风险监控**：实时风险评估和智能预警系统
- **📈 数据质量监控**：全面的数据质量检测和验证机制
...
- **🆕 纯Loguru日志系统**：完全基于Loguru的现代化日志架构，支持结构化日志、异步处理和智能错误恢复
- **🆕 智能化运维**：自动调优、性能预测和异常检测，降低运维成本

## 系统架构

FactorWeave-Quant ‌采用现代化的分层架构设计，主要组件包括：
...
4. **核心服务层**：
   ...
   - **DataImportExecutionEngine**：🆕 DuckDB专业数据导入引擎
   - **AIPredictionService**：🆕 AI智能预测和参数优化服务
   - **UnifiedPerformanceMonitor**：🆕 统一性能监控系统
   - **EnhancedRiskMonitor**：🆕 增强风险监控和预警系统
   - **MultiLevelCacheManager**：🆕 多级缓存管理器
   - **EnhancedEventBus**：🆕 增强事件总线系统
   - **EnhancedDistributedService**：🆕 分布式任务执行服务
...
│              UI Layer (PyQt5)               │
│  ┌─────────────────┬─────────────────────┐   │
│  │  MainWindow     │   Analysis Tabs     │   │
│  │  Coordinator    │   (8个专业标签页)    │   │
│  └─────────────────┴─────────────────────┘   │
├─────────────────────────────────────────────┤
│            Service Layer (核心服务)          │
│  ┌───────────┬──────────┬──────────────┐    │
│  │AnalysisS. │StockS.   │ChartService  │    │
│  │TradingS.  │ThemeS.   │DataManager   │    │
│  └───────────┴──────────┴──────────────┘    │
├─────────────────────────────────────────────┤
│         Business Logic (业务逻辑)            │
│  ┌───────────┬──────────┬──────────────┐    │
│  │Pattern    │Strategy  │Technical     │    │
│  │Manager    │Engine    │Analysis      │    │
│  └───────────┴──────────┴──────────────┘    │
├─────────────────────────────────────────────┤
│           Data Layer (数据层)               │
│  ┌───────────┬──────────┬──────────────┐    │
│  │FactorWeave-Quant     │AkShare   │Database      │    │
│  │Engine     │Source    │SQLite        │    │
│  └───────────┴──────────┴──────────────┘    │
└─────────────────────────────────────────────┘
```
...
│                   数据层 (异步处理)                           │
├─────────────────────────────────────────────────────────────┤
│  DataManager → AsyncDataProcessor → Cache → FactorWeave-QuantAPI      │
└─────────────────────────────────────────────────────────────┘
```
...
```
用户操作 → 配置管理(SQLite) → 业务逻辑 → 分析计算(DuckDB) → 结果展示
    ↓           ↓                ↓            ↓            ↓
 界面状态    系统设置         策略执行      指标计算      可视化
 插件配置    主题管理         信号生成      形态识别      报表生成
```

### 🆕 纯Loguru日志系统

FactorWeave-Quant v2.0 引入了基于 [Loguru](https://loguru.readthedocs.io/) 的现代化日志架构：
...
详见：[Loguru迁移指南](docs/loguru_migration_guide.md)
...
### 🆕 7. DuckDB专业数据导入系统
- **高性能导入引擎**：基于DuckDB的高速数据导入和处理
- **多数据源支持**：支持CSV、JSON、Parquet等多种格式
- **智能配置管理**：自动优化导入参数和性能配置
...
### 🆕 10. 增强风险监控系统
- **实时风险评估**：多维度风险指标实时计算
- **投资组合分析**：集中度风险、波动性风险、流动性风险评估
- **智能预警**：基于规则引擎的智能风险预警
- **风险控制策略**：自动风险控制和止损机制
- **合规检查**：投资合规性检查和报告

### 🆕 11. 多级缓存系统
- **内存缓存**：高速内存缓存，支持LRU和TTL策略
- **磁盘缓存**：持久化磁盘缓存，支持压缩和加密
- **智能预取**：基于访问模式的智能数据预取
- **缓存统计**：详细的缓存命中率和性能统计
- **自动清理**：基于策略的自动缓存清理和优化

### 🆕 12. 分布式任务执行
- **智能负载均衡**：多种负载均衡策略，自动选择最优节点
- **故障转移**：自动故障检测和任务转移
- **动态扩缩容**：根据负载自动调整节点数量
- **任务优先级**：支持任务优先级和资源需求管理
- **监控和日志**：全面的分布式系统监控和日志记录

## 🔧 性能优化系统

FactorWeave-Quant ‌项目实现了全面的性能优化，显著提升了UI响应速度和用户体验：
...
FactorWeave-Quant ‌项目的技术指标系统已完成从混合实现迁移至统一、动态、数据库驱动的全新架构。
...
FactorWeave-Quant ‌项目采用模块化设计，各功能模块以目录形式清晰组织，便于开发和维护。以下是完整的目录结构及其功能说明：
...
│   │   ├── __init__.py
│   │   ├── data_access.py               # 数据访问
│   │   └── FactorWeave-Quant_data_manager.py       # FactorWeave-Quant数据管理器
...
│   ├── FactorWeave-Quant_source.py                 # FactorWeave-Quant数据源
...
├── db/                                  # 数据库相关
│   ├── __init__.py
│   ├── FactorWeave-Quant_system.db                 # 系统数据库
│   ├── indicators.db                    # 指标数据库
│   ├── init_db.py                       # 数据库初始化
...
│   ├── FactorWeave-Quant-docs/                     # FactorWeave-Quant文档
│   │   ├── examples/                    # 示例
...
- `FactorWeave-Quant_system.db` - 系统配置数据库，存储系统设置
...
4. **core/services/unified_data_manager.py** - 统一数据管理器
   - 提供统一的数据访问接口
   - 整合多个数据源（FactorWeave-Quant、东方财富、新浪、同花顺等）
   - 实现数据缓存和异步加载
   - 处理数据预处理和验证
...
FactorWeave-Quant量化框架将交易系统拆分为多个独立组件：
...
- ✅ **WebGPU硬件加速系统**：完整的GPU加速渲染引擎（Phase 1-3已完成）
  - **WebGPU环境检测**：多层级硬件兼容性检查和自动降级机制
  - **GPU交互引擎**：硬件加速的缩放、平移、动画和视图变换
  - **GPU十字光标引擎**：实时鼠标跟踪、多样式十字光标、数据捕捉
  - **渲染器集成**：完全向下兼容的WebGPU图表渲染器
  - **性能监控**：实时性能统计和自动后端切换
