...
try:
    ...
    from core.asset_type_identifier import AssetTypeIdentifier, identify_asset_type
    from core.asset_database_manager import AssetSeparatedDatabaseManager
    from core.data_router import DataRouter, DataRequest, DataSource
...
class AdjustedArchitectureTestSuite:
    """调整后的架构测试套件"""
        ...
        try:
            ...
            for symbol, expected_type in test_cases:
                ...
                if result == expected_type:
                    ...
                    print(f"  WARNING {symbol} 识别结果: {result.value}, 期望: {expected_type.value}")

            # 测试便捷函数
            result = identify_asset_type('000001')
            print(f"  OK 便捷函数测试: 000001 -> {result.value}")
    ...
    def test_asset_database_manager(self) -> bool:
        ...
        try:
            ...
            try:
                # 测试获取数据库连接
                db_path, asset_type = db_manager.get_database_for_symbol(test_symbol)
                print(f"  OK 获取数据库连接成功: {test_symbol} -> {db_path} ({asset_type.value})")
            except Exception as e:
                print(f"  WARNING 数据库连接测试失败: {e}")

            # 测试统计信息
            stats = db_manager.get_database_statistics()
            print(f"  OK 数据库统计: 总数据库数={stats.get('total_databases', 0)}")
    ...
    def test_data_router(self) -> bool:
        ...
        try:
            ...
            route_result = router.route_data_request(request)
            ...
            batch_results = router.batch_route_requests(requests)
    ...
    def test_asset_aware_data_manager(self) -> bool:
        ...
        try:
            ...
            for symbol in self.test_data['symbols'][:3]:  # 测试前3个
                try:
                    # 使用实际的方法签名
                    result = data_manager.get_cross_asset_data([symbol])
            ...
            try:
                ...
                print(f"  OK 数据管理器统计: 数据库数={stats.get('database_statistics', {}).get('total_databases', 0)}")
            except Exception as e:
    ...
    def test_data_standardization_engine(self) -> bool:
        ...
        try:
            ...
            result = engine.standardize_data(
                raw_data, DataSource.TONGDAXIN, DataType.HISTORICAL_KLINE,
                AssetType.STOCK_A, '000001'
            )
            if result.success:
                data_count = len(result.data) if result.data is not None else 0
                print(f"  OK 数据标准化成功: {data_count} 条记录")
                print(f"  OK 质量评分: {result.quality_score}")
            else:
                issues = ', '.join(result.quality_issues) if result.quality_issues else '未知错误'
                print(f"  ERROR 数据标准化失败: {issues}")
                return False
            ...
            for source, asset_type in test_sources:
                try:
                    test_result = engine.standardize_data(
                        raw_data, source, DataType.HISTORICAL_KLINE, asset_type, '000001'
                    )
    ...
    def test_cross_asset_query_engine(self) -> bool:
        ...
        try:
            ...
            # 测试查询请求
            from core.cross_asset_query_engine import CrossAssetQueryRequest, QueryFilter, QueryType

            query_request = CrossAssetQueryRequest(
                query_type=QueryType.CROSS_ASSET,
    ...
    def test_database_maintenance_engine(self) -> bool:
        ...
        try:
            ...
            if task:
                print(f"  OK 任务状态: {task.get('status', 'unknown')}")
            ...
            # 检查任务完成情况
            completed_tasks = [t for t in maintenance_engine.list_active_tasks() if t.get('status') == 'completed']
    ...
    def test_data_missing_manager(self) -> bool:
        ...
        try:
            ...
            # 测试插件建议
            suggestions = missing_manager.suggest_data_sources('000001', DataType.HISTORICAL_KLINE)
            print(f"  OK 插件建议: {len(suggestions)} 个")

            # 测试插件可用性
            plugin_status = missing_manager.get_plugin_availability()
            available_count = sum(1 for is_available in plugin_status.values() if is_available)
        ...
        try:
            ...
            if 'standardization_engine' in self.components:
                ...
                std_result = self.components['standardization_engine'].standardize_data(
                    raw_data, DataSource.TONGDAXIN, DataType.HISTORICAL_KLINE,
                    AssetType.STOCK_A, 'INTEGRATION_TEST'
                )
            ...
            if 'standardization_engine' in self.components:
                ...
                for i in range(100):
                    self.components['standardization_engine'].standardize_data(
                        test_data, DataSource.TONGDAXIN, DataType.HISTORICAL_KLINE,
                        AssetType.STOCK_A, 'PERF_TEST'
                    )
                end_time = time.time()
        ...
        try:
            ...
            if 'identifier' in self.components:
                try:
                    ...
                    print(f"    OK 无效符号处理: {result.value}")
                except Exception as e:
            ...
            if 'db_manager' in self.components:
                try:
                    # 尝试获取无效资产类型的数据库
                    result = self.components['db_manager'].get_database_for_asset_type(AssetType.STOCK)
            ...
            if 'standardization_engine' in self.components:
                try:
                    ...
                    result = self.components['standardization_engine'].standardize_data(
                        invalid_data, DataSource.TONGDAXIN, DataType.HISTORICAL_KLINE,
                        AssetType.STOCK_A, 'INVALID'
                    )
...
def main():
    """主函数"""
    test_suite = AdjustedArchitectureTestSuite()
