...
class StockScreeningWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.strategy_type_combo.addItems(["技术指标筛选", "基本面筛选", "资金流向筛选", "综合筛选"])
        ...
        self.template_combo.addItems(["", "强势股筛选", "价值投资", "成长股筛选", "反转策略", "动量策略"])
    ...
    def update_result_table(self, results: pd.DataFrame):
        ...
        for i, row in results.iterrows():
            ...
            self.result_table.setItem(i, 3, QTableWidgetItem(f"{row['price']:.2f}"))
            self.result_table.setItem(i, 4, QTableWidgetItem(f"{row['change']:.2f}%"))
            self.result_table.setItem(i, 5, QTableWidgetItem(f"{row['pe']:.2f}"))
            self.result_table.setItem(i, 6, QTableWidgetItem(f"{row['pb']:.2f}"))
            self.result_table.setItem(i, 7, QTableWidgetItem(f"{row['roe']:.2f}%"))
            self.result_table.setItem(i, 8, QTableWidgetItem(f"{row['main_force']:.2f}"))
            self.result_table.setItem(i, 9, QTableWidgetItem(f"{row['north_money']:.2f}"))
