...
# -*- coding: utf-8 -*-
"""
HIkyuu-UI 插件系统全面分析工具

该工具对所有现有插件进行全面检测分析，包括：
1. 插件识别和加载状态
2. 业务框架完整性检查
3. 系统框架兼容性验证
4. 缺失内容识别
...
class PluginAnalyzer:
    ...
    def initialize(self):
        """初始化分析器"""
        try:
            logger.info("🔧 初始化插件分析器...")

            # 初始化统一数据管理器
            self.unified_manager = get_unified_data_manager()
    ...
    def _analyze_plugin_loading_status(self):
        ...
        logger.info(f"  📋 数据库中插件总数: {len(db_plugins)}")
        logger.info(f"  🔌 已加载插件总数: {len(loaded_plugins)}")
        logger.info(f"  📊 数据源插件总数: {len(data_source_plugins)}")
        ...
        for plugin_data in db_plugins:
            ...
            if plugin_status == 'enabled' and not analysis['is_loaded']:
                analysis['issues'].append("插件状态为enabled但未被加载")
                self.analysis_results['issues'].append(f"{plugin_name}: 插件状态为enabled但未被加载")

            self.analysis_results['plugins'][plugin_name] = analysis

    def _analyze_business_framework(self):
        """分析业务框架完整性"""
        logger.info("2️⃣ 分析业务框架完整性...")
        ...
        for plugin_id, plugin_instance in data_source_plugins.items():
            ...
            if missing_methods:
                issue = f"缺少业务方法: {', '.join(missing_methods)}"
            ...
            try:
                if hasattr(plugin_instance, 'get_supported_data_types'):
                    ...
                    if missing_basic_types:
                        issue = f"缺少基本数据类型支持: {[str(dt) for dt in missing_basic_types]}"
                        plugin_analysis['issues'].append(issue)
                        self.analysis_results['issues'].append(f"{plugin_id}: {issue}")
                else:
                    plugin_analysis['issues'].append("未实现get_supported_data_types方法")

            except Exception as e:
                plugin_analysis['issues'].append(f"获取支持的数据类型失败: {e}")

    def _analyze_system_framework(self):
        """分析系统框架兼容性"""
        logger.info("3️⃣ 分析系统框架兼容性...")

        uni_plugin_manager = self.unified_manager.get_uni_plugin_manager()
        data_source_plugins = uni_plugin_manager.plugin_center.data_source_plugins

        # 检查插件是否符合IDataSourcePlugin接口
        for plugin_id, plugin_instance in data_source_plugins.items():
            plugin_analysis = self.analysis_results['plugins'][plugin_id]
            ...
            if not interface_compliance:
                issue = "不符合IDataSourcePlugin接口规范"
                plugin_analysis['issues'].append(issue)
                self.analysis_results['issues'].append(f"{plugin_id}: {issue}")
            ...
            if not has_health_check:
                plugin_analysis['issues'].append("缺少health_check方法")

    def _identify_missing_content(self):
        """识别缺失内容"""
        logger.info("4️⃣ 识别缺失内容...")
    ...
    def _print_summary(self):
        """打印分析摘要"""
        logger.info("\n" + "="*60)
        logger.info("📋 HIkyuu-UI 插件系统分析摘要")
        logger.info("="*60)

        summary = self.analysis_results['summary']

        logger.info(f"📊 插件统计:")
        logger.info(f"  数据库中插件总数: {summary.get('total_db_plugins', 0)}")
        logger.info(f"  已加载插件总数: {summary.get('total_loaded_plugins', 0)}")
        logger.info(f"  数据源插件总数: {summary.get('total_data_source_plugins', 0)}")

        logger.info(f"\n🎯 能力统计:")
        capabilities = summary.get('capabilities', {})
        logger.info(f"  资产列表提供者: {len(capabilities.get('asset_list_providers', []))}")
        logger.info(f"  实时行情提供者: {len(capabilities.get('real_time_providers', []))}")
        logger.info(f"  历史数据提供者: {len(capabilities.get('historical_data_providers', []))}")
        logger.info(f"  资金流提供者: {len(capabilities.get('fund_flow_providers', []))}")

        logger.info(f"\n⚠️ 问题统计:")
        logger.info(f"  总问题数: {summary.get('total_issues', 0)}")
        logger.info(f"  关键问题数: {summary.get('critical_issues', 0)}")
        ...
        if self.analysis_results['issues']:
            ...
            if len(self.analysis_results['issues']) > 10:
                logger.info(f"  ... 还有 {len(self.analysis_results['issues']) - 10} 个问题")
...
def main():
    """主函数"""
    logger.info("🚀 启动HIkyuu-UI插件系统全面分析...")
