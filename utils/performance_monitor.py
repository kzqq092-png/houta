...
class PerformanceMonitor(QObject):
    ...
    def start_monitoring(self):
        ...
        try:
            ...
            self.monitor_thread.started.connect(self._monitor_performance)
    ...
    def _check_performance(self):
        ...
        try:
            if self._metrics['cpu_usage'] > self.cpu_threshold:
                self.alert_triggered.emit(f"CPU使用率过高: {self._metrics['cpu_usage']}%")
                
            if self._metrics['memory_usage'] > self.memory_threshold:
                self.alert_triggered.emit(f"内存使用率过高: {self._metrics['memory_usage']}%")
                
            if self._metrics['disk_usage'] > self.disk_threshold:
                self.alert_triggered.emit(f"磁盘使用率过高: {self._metrics['disk_usage']}%")
