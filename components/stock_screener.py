...
class StockScreenerWidget(QWidget):
    ...
    def init_ui(self):
        ...
        try:
            ...
            self.strategy_type.currentTextChanged.connect(self.on_strategy_changed)
            ...
            self.result_table.itemSelectionChanged.connect(self.on_stock_selected)
    ...
    def load_strategy(self):
        ...
        try:
            ...
            if file_path:
                ...
                # 更新策略类型
                index = self.strategy_type.findText(strategy_data['strategy_type'])
                if index >= 0:
                ...
                QMessageBox.information(self, "成功", "选股策略加载成功")
                
        except Exception as e:
            self.log_manager.log(f"加载选股策略失败: {str(e)}", LogLevel.ERROR)
            QMessageBox.critical(self, "错误", f"加载选股策略失败: {str(e)}")
    ...
    def on_template_changed(self, template_name: str):
        ...
        try:
            ...
            # 加载模板
            strategy_type, conditions = self.screener.load_strategy(template_name)
        ...
        try:
            ...
            if strategy_type == "技术指标选股":
                ...
                if 'ma_condition' in conditions:
                    index = self.ma_condition.findText(conditions['ma_condition'])
                    if index >= 0:
                        self.ma_condition.setCurrentIndex(index)
                        
                if 'macd_condition' in conditions:
                    index = self.macd_condition.findText(conditions['macd_condition'])
                    if index >= 0:
                ...
                if 'rsi_condition' in conditions:
                    index = self.rsi_condition.findText(conditions['rsi_condition'])
                    if index >= 0:
    ...
    def save_template(self):
        ...
        try:
            ...
            if ok and template_name:
                ...
                # 保存模板
                self.screener.save_strategy(strategy_type, conditions, template_name)
    ...
    def add_sorting_functions(self):
        ...
        try:
            ...
            self.sort_direction.currentTextChanged.connect(self.on_sort_changed)
    ...
    def screen_stocks(self):
        ...
        try:
            ...
            for i, stock in enumerate(stock_list):
                ...
                # 更新进度
                progress.setValue(i)
                progress.setLabelText(f"正在筛选股票: {stock['code']} - {stock['name']}")
        ...
        try:
            ...
            for i, stock in enumerate(stocks):
                try:
                    ...
                    # 获取股票数据
                    data = self.screener.data_manager.get_stock_data(stock['code'])
                    ...
                    if self.strategy_type == "技术指标选股":
                        ...
                        score = self.screener.calculate_comprehensive_score(data, self.conditions)
                        if score > 0:
                            results.append({
                                'code': stock['code'],
                                'name': stock['name'],
                    ...
                    self.screener.log_manager.log(f"处理股票 {stock['code']} 失败: {str(e)}", LogLevel.ERROR)
                    continue
