...
class ModelAnalysis:
    ...
    def plot_model_performance(self, performance_metrics, model_names, figsize=(12, 8)):
        ...
        try:
            ...
            # 绘制准确率
            accuracy = [performance_metrics[model]['accuracy'] for model in model_names]
            axes[0].bar(model_names, accuracy, color='blue')
            axes[0].set_title('Accuracy')
            axes[0].set_ylim(0, 1)
            
            # 绘制精确率
            precision = [performance_metrics[model]['precision'] for model in model_names]
            axes[1].bar(model_names, precision, color='green')
            axes[1].set_title('Precision')
            axes[1].set_ylim(0, 1)
            
            # 绘制召回率
            recall = [performance_metrics[model]['recall'] for model in model_names]
