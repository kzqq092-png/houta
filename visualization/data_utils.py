...
class DataUtils:
    def __init__(self, log_manager=None):
        ...
        if self.log_manager is None:
            try:
                ...
                self.log_manager = SimpleLogger()

    def load_data(self, data):
        """Load and prepare data for analysis"""
        if isinstance(data, pd.DataFrame):
            self.data = data
            self.log_manager.info(f"加载DataFrame数据，共{len(data)}行")
        else:
            ...
            self.log_manager.info(f"转换数据为DataFrame格式，共{len(self.data)}行")
        return self.data

    def calculate_indicators(self):
        """计算技术指标 - 使用统一的指标计算模块"""
        if self.data is None:
            self.log_manager.warning("数据为空，无法计算指标")
            return None
    ...
    def _calculate_simple_indicators_fallback(self, df):
        ...
        self.log_manager.info("后备指标计算完成")
        return df
    ...
    def get_signals(self, df=None):
        ...
        if df is None:
            self.log_manager.warning("无法生成交易信号，数据为空")
            return None
        ...
        self.log_manager.info(f"生成交易信号完成，共{signal_count}个信号")
        return signals
