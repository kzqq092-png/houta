...
def calculate_returns(df, position_col='position', price_col='close',
    ...
    # 根据持仓计算每日收益
    result_df['daily_return'] = result_df['price_change_pct'] * \
        result_df[position_col].shift(1)
    ...
    if include_commission:
        ...
        # 从收益中减去交易成本
        result_df['daily_return'] = result_df['daily_return'] - \
            result_df['commission']

    # 计算累积收益
    result_df['cumulative_return'] = (
        1 + result_df['daily_return']).cumprod() - 1
...
def calculate_performance_metrics(returns_df, daily_return_col='daily_return', risk_free_rate=0.02,
    ...
    ) * np.sqrt(trading_days) if len(downside_returns) > 0 else 0

    # 计算夏普比率
    excess_returns = daily_returns - daily_rf
    sharpe_ratio = (excess_returns.mean() * trading_days) / \
    ...
    # 计算胜率
    win_rate = (daily_returns > 0).sum() / \
        len(daily_returns) if len(daily_returns) > 0 else 0

    # 计算盈亏比
    avg_win = daily_returns[daily_returns > 0].mean() if len(
        daily_returns[daily_returns > 0]) > 0 else 0
    avg_loss = daily_returns[daily_returns < 0].mean() if len(
        daily_returns[daily_returns < 0]) > 0 else 0
    ...
    information_ratio = (daily_returns.mean() * trading_days) / (
        daily_returns.std() * np.sqrt(trading_days)) if daily_returns.std() != 0 else 0

    # 计算Beta和Alpha（这里简化，使用市场收益为risk_free_rate）
    # 在实际应用中应该使用实际的市场收益
    beta = 0  # 简化，实际中需要计算与市场的相关性
    alpha = annualized_return - \
        (risk_free_rate + beta * (0.08 - risk_free_rate))  # 假设市场风险溢价为8%
    ...
    ) * np.sqrt(trading_days) if len(daily_returns[daily_returns <= var_95]) > 0 else 0
...
def plot_equity_curve(returns_df, benchmark_df=None, figsize=(15, 8)):
    ...
    if 'cumulative_return' not in returns_df.columns:
        if 'daily_return' in returns_df.columns:
            returns_df['cumulative_return'] = (
                1 + returns_df['daily_return']).cumprod() - 1
        else:
    ...
    # 绘制策略收益曲线
    plt.plot(returns_df.index,
             returns_df['cumulative_return'], label='Strategy', color='blue')
        ...
        if 'cumulative_return' not in benchmark_df.columns:
            if 'daily_return' in benchmark_df.columns:
                benchmark_df['cumulative_return'] = (
                    1 + benchmark_df['daily_return']).cumprod() - 1
            else:
                raise ValueError(
                    "基准DataFrame中缺少'cumulative_return'或'daily_return'列")

        plt.plot(benchmark_df.index,
                 benchmark_df['cumulative_return'], label='Benchmark', color='red')
    ...
    plt.gca().yaxis.set_major_formatter(
        plt.FuncFormatter(lambda x, _: '{:.2%}'.format(x)))
    ...
    if 'cumulative_return' in returns_df.columns:
        ...
        plt.annotate(f'Max Drawdown: {max_dd:.2%}',
                     xy=(max_dd_idx,
                         returns_df.loc[max_dd_idx, 'cumulative_return']),
                     xytext=(
                         max_dd_idx, returns_df.loc[max_dd_idx, 'cumulative_return'] - 0.1),
...
def plot_monthly_returns_heatmap(returns_df, daily_return_col='daily_return', figsize=(15, 8)):
    ...
    plt.pcolormesh(monthly_returns_pivot, cmap=cmap,
    ...
    # 设置坐标轴标签
    plt.xticks(np.arange(0.5, 12.5), [
               'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'])
    plt.yticks(np.arange(0.5, len(monthly_returns_pivot.index) + 0.5),
               monthly_returns_pivot.index.tolist())
...
def create_performance_report(returns_df, benchmark_df=None, file_path=None):
    ...
    if benchmark_df is not None:
        ...
        relative_metrics = {
            ...
            (np.sqrt(
                ((returns_df['daily_return'] - benchmark_df['daily_return'])**2).mean()) * np.sqrt(252))
