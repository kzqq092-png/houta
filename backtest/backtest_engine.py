...
class StrategyBacktester:
    ...
    def run_backtest(self, signal_col='signal', price_col='close', stop_loss_pct=None, 
        ...
        for i in range(len(results)):
            ...
            if current_position != 0:
                results.loc[results.index[i], 'holding_periods'] = (current_date - entry_date).days
            ...
            if (current_position == 0 and current_signal != 0) or (current_position != 0 and 
                ...
                if current_position == 0:  # 开仓
                    ...
                    if current_position == 1:
                        trade_profit = (exit_price / entry_price - 1) * self.position_size * current_capital
                    else:
                        trade_profit = (1 - exit_price / entry_price) * self.position_size * current_capital
                    ...
                    results.loc[results.index[i], 'exit_reason'] = exit_reason if exit_triggered else 'Signal'
                    ...
                    if self.trades:
                        ...
                        self.trades[-1]['return'] = trade_profit / (self.trades[-1]['size'])
            ...
            if current_position != 0:
                if current_position == 1:
                    unrealized_profit = (current_price / entry_price - 1) * self.position_size * current_capital
                else:
                    unrealized_profit = (1 - current_price / entry_price) * self.position_size * current_capital
            ...
            # 计算收益率
            if i > 0:
                results.loc[results.index[i], 'returns'] = results.loc[results.index[i], 'equity'] / results.loc[results.index[i-1], 'equity'] - 1
    ...
    def calculate_metrics(self, risk_free_rate=0.02, trading_days=252):
        ...
        sharpe_ratio = (returns.mean() - daily_risk_free) * trading_days / annualized_volatility
        ...
        if self.trades:
            ...
            winning_returns = [trade['return'] for trade in self.trades if trade.get('profit', 0) > 0]
            losing_returns = [trade['return'] for trade in self.trades if trade.get('profit', 0) <= 0]
    ...
    def plot_results(self, benchmark_data=None, figsize=(15, 10)):
        ...
        # 绘制权益曲线
        axes[0].plot(self.results.index, self.results['equity'], label='Strategy Equity')
        ...
        if benchmark_data is not None:
            if 'close' in benchmark_data.columns:
                # 调整基准数据规模与策略相同
                benchmark_scaled = benchmark_data['close'] / benchmark_data['close'].iloc[0] * self.initial_capital
                axes[0].plot(benchmark_data.index, benchmark_scaled, label='Benchmark', alpha=0.7)
                axes[0].legend()
        
        # 绘制持仓
        axes[1].plot(self.results.index, self.results['position'], label='Position')
        ...
        axes[2].fill_between(self.results.index, drawdown, 0, color='red', alpha=0.3)
        axes[2].plot(self.results.index, drawdown, color='red', label='Drawdown')
