    ...
    def draw_overview(self, ax, kdata):
        ...
        try:
            ...
            for i in range(len(kdata)):
                ...
                ax.plot([x[i], x[i]], [lows.iloc[i], highs.iloc[i]], color=color, linewidth=0.5)
                ax.plot([x[i], x[i]], [opens.iloc[i], closes.iloc[i]], color=color, linewidth=1.5)
