...
class AnalysisWidget(QWidget):
    ...
    def __init__(self, config_manager: Optional[ConfigManager] = None):
        ...
        # 应用主题
        self.theme_manager = get_theme_manager(config_manager or ConfigManager())
    ...
    def calculate_ma(self):
        ...
        try:
            ...
            for i, spin in enumerate(self.ma_periods):
                ...
                status_item.setForeground(Qt.red if color == "red" else Qt.green)
                self.indicator_table.setItem(row, 2, status_item)
                
                suggestion_item = QTableWidgetItem(suggestion)
                suggestion_item.setForeground(Qt.red if color == "red" else Qt.green)
    ...
    def calculate_comprehensive(self):
        ...
        try:
            ...
            # 判断MACD趋势
            macd_trend = "多头" if float(macd.dif[-1]) > float(macd.dea[-1]) else "空头"
    ...
    def analyze_price_trend(self, period: int, threshold: float):
        ...
        try:
            ...
            # 计算趋势强度
            strength = abs(float(close[-1]) - float(ma[-1])) / float(ma[-1]) * 100
    ...
    def analyze_volume_trend(self, period: int, threshold: float):
        ...
        try:
            ...
            # 计算趋势强度
            strength = abs(float(volume[-1]) - float(ma[-1])) / float(ma[-1]) * 100
    ...
    def analyze_industry_flow(self):
        ...
        try:
            ...
            # 获取所有行业板块
            industries = [block for block in sm.get_block_list() if block.type == "行业"]
            ...
            for industry in industries:
                try:
                    ...
                    log_manager.log(f"计算行业 {industry.name} 资金流向失败: {str(e)}", "error")
                    continue
            ...
            for i, flow in enumerate(industry_flows):
                self.industry_flow_table.setItem(i, 0, QTableWidgetItem(flow['name']))
                
                main_flow_item = QTableWidgetItem(f"{flow['main_flow']:+.2f}")
                main_flow_item.setForeground(QColor("red" if flow['main_flow'] > 0 else "green"))
                self.industry_flow_table.setItem(i, 1, main_flow_item)
                
                super_flow_item = QTableWidgetItem(f"{flow['super_flow']:+.2f}")
                super_flow_item.setForeground(QColor("red" if flow['super_flow'] > 0 else "green"))
                self.industry_flow_table.setItem(i, 2, super_flow_item)
                
                big_flow_item = QTableWidgetItem(f"{flow['big_flow']:+.2f}")
                big_flow_item.setForeground(QColor("red" if flow['big_flow'] > 0 else "green"))
                self.industry_flow_table.setItem(i, 3, big_flow_item)
                
                mid_flow_item = QTableWidgetItem(f"{flow['mid_flow']:+.2f}")
                mid_flow_item.setForeground(QColor("red" if flow['mid_flow'] > 0 else "green"))
    ...
    def analyze_concept_flow(self):
        ...
        try:
            ...
            # 获取所有概念板块
            concepts = [block for block in sm.get_block_list() if block.type == "概念"]
            ...
            for concept in concepts:
                try:
                    ...
                    log_manager.log(f"计算概念 {concept.name} 资金流向失败: {str(e)}", "error")
                    continue
            ...
            for i, flow in enumerate(concept_flows):
                self.concept_flow_table.setItem(i, 0, QTableWidgetItem(flow['name']))
    ...
    def analyze_north_flow(self):
        ...
        try:
            ...
            for i in range(len(dates)):
                ...
                sh_flow_item.setForeground(QColor("red" if sh_flows[i] > 0 else "green"))
                self.north_flow_table.setItem(i, 1, sh_flow_item)
                
                sz_flow_item = QTableWidgetItem(f"{sz_flows[i]:+.2f}")
                sz_flow_item.setForeground(QColor("red" if sz_flows[i] > 0 else "green"))
                self.north_flow_table.setItem(i, 2, sz_flow_item)
                
                total_flow_item = QTableWidgetItem(f"{total_flows[i]:+.2f}")
                total_flow_item.setForeground(QColor("red" if total_flows[i] > 0 else "green"))
    ...
    def analyze_hotspot_sectors(self):
        ...
        try:
            ...
            for block in sm.get_block_list():
                try:
                    ...
                    for stock in stocks:
                        ...
                        # 计算5日涨跌幅
                        five_day_change = (close - float(kdata[0].close)) / float(kdata[0].close) * 100
                    ...
                    log_manager.log(f"计算板块 {block.name} 统计失败: {str(e)}", "error")
                    continue
            ...
            for i, sector in enumerate(sectors):
                # 添加板块名称
                self.hotspot_table.setItem(i, 0, QTableWidgetItem(sector['name']))
                
                # 添加板块涨跌幅
                change_item = QTableWidgetItem(f"{sector['change']:+.2f}%")
                change_item.setForeground(QColor("red" if sector['change'] > 0 else "green"))
        ...
        try:
            ...
            for block in sm.get_block_list():
                ...
                try:
                    ...
                    log_manager.log(f"计算主题 {block.name} 统计失败: {str(e)}", "error")
                    continue
    ...
    def analyze_leading_stocks(self):
        ...
        try:
            ...
            for i, leader in enumerate(leaders):
                # 添加股票名称
                self.leader_table.setItem(i, 0, QTableWidgetItem(leader['name']))
                
                # 添加所属板块
                self.leader_table.setItem(i, 1, QTableWidgetItem(leader['block']))
        ...
        try:
            ...
            for block in sm.get_block_list():
                try:
                    ...
                    log_manager.log(f"计算板块 {block.name} 轮动分析失败: {str(e)}", "error")
                    continue
            ...
            for i, rotation in enumerate(rotations):
                # 添加板块名称
                self.rotation_table.setItem(i, 0, QTableWidgetItem(rotation['name']))
                ...
                flow_item.setForeground(QColor("red" if rotation['flow'] > 0 else "green"))
