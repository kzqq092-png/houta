...
class StockDetailDialog(QDialog):
    ...
    def __init__(self, stock_data: Dict[str, Any], parent=None):
        ...
        self.setWindowTitle(f"股票详情 - {stock_data['name']} ({stock_data['code']})")
    ...
    def create_history_tab(self) -> QWidget:
        ...
        try:
            ...
            for row, data in enumerate(history_data[:20]):
                ...
                table.setItem(row, 4, QTableWidgetItem(f"{data['close']:,.2f}"))
                table.setItem(row, 5, QTableWidgetItem(f"{data['volume']:,.0f}"))
        ...
        try:
            ...
            data = {
                '基本信息': pd.DataFrame([
                    ...
                    {'项目': '上市日期', '值': self.stock_data.get('list_date', '未知')},
                    {'项目': '总股本', '值': self.stock_data.get('total_shares', 0)},
                    {'项目': '流通股本', '值': self.stock_data.get('circulating_shares', 0)},
