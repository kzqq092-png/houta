    ...
    try:
        ...
        def compute_feature_scores(method_name, scorer, X, y):
            ...
            if method_name == 'f_classif':
                ...
                selector.fit(X, y)
                scores = np.flip(np.array(range(1, len(selector.ranking_) + 1)) / len(selector.ranking_))
        ...
        # 互信息（对于非线性关系更有效）
        mi_scores = compute_feature_scores('mutual_info', mutual_info_classif, X_values, y)
        ...
        # 递归特征消除
        rfe_scores = compute_feature_scores('recursive_elimination', None, X_values, y)
        ...
        def compute_stability_iteration(i):
            ...
            for train_idx, test_idx in cv.split(X_values, y):
                ...
                fold_importance = compute_feature_scores('random_forest', None, X_train, y_train)
        ...
        # 按最终得分排序
        feature_importance = feature_importance.sort_values('final_score', ascending=False)
...
def integrate_enhanced_features(df):
    ...
    # 如果未计算基础指标，先计算
    from .basic_indicators import calculate_base_indicators, calculate_market_sentiment
    from .basic_indicators import create_advanced_nonlinear_features, create_time_series_features
...
def select_features_importance(X, y, n_features=20, model_type='random_forest', random_state=42):
    ...
    if model_type == 'random_forest':
        ...
        raise ValueError("不支持的模型类型，请选择'random_forest'、'mutual_info'或'f_classif'")
...
def remove_redundant_features(X, y, threshold=0.7, target_col=None):
    ...
    # 获取特征之间的相关矩阵
    feature_corr = corr_matrix.drop(target_col, axis=0).drop(target_col, axis=1)
