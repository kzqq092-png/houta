...
class AdaptiveTakeProfit(ProfitGoalBase):
    ...
    def _calculate(self, datetime, stock, price, risk, part_from):
        ...
        try:
            ...
            # 5. 选择最终的止盈价格
            profit_price = self._select_profit_price(price, profits, position, trend)
    ...
    def _calculate_trailing_profit(self, price, position):
        ...
        try:
            ...
            profit_distance = position['highest_price'] * self.get_param("trailing_profit")
        ...
        try:
            ...
            if price > position['entry_price']:
                ...
                if profit_ratio > self.get_param("profit_lock"):
                    # 锁定部分利润
                    lock_price = position['entry_price'] * (1 + self.get_param("profit_lock"))
