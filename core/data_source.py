    ...
    def get_market_sentiment(self) -> dict:
        try:
            ...
            if kdata is None or kdata.empty():
                self.logger.error("[Hikyuu] 市场情绪K线数据为空")
                return self._empty_sentiment()
            closes = [k.closePrice for k in kdata]
            vols = [k.volume for k in kdata]
            ...
            flow = sum((closes[i] - closes[i-1]) * vols[i]
                       for i in range(1, len(closes)))
            main_fund_flow = flow / 1e8  # 单位：亿元
            pct_chg = [(closes[i] - closes[i-1]) / closes[i-1]
                       for i in range(1, len(closes))]
            sector_heat = sum(1 for x in pct_chg if x > 0.02) / \
                len(pct_chg) * 100 if pct_chg else None
            return {
                'sentiment_index': sentiment_index,
                'advance_decline': {'advance': ups, 'decline': downs, 'unchanged': total - ups - downs},
                'volume_ratio': 1.0,
            ...
            self.logger.error(f"Hikyuu获取市场情绪失败: {str(e)}")
            return self._empty_sentiment()
