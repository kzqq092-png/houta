...
class EnhancedMoneyManager(MoneyManagerBase):
    ...
    def _calculate_position_scale(self):
        ...
        try:
            ...
            # 4. 根据风险预算调整
            remaining_budget = self.get_param(
                "risk_budget") - self.risk_budget_used
        ...
        try:
            ...
            for stock, position in self.positions.items():
                if position['stop_loss'] > 0:
                    risk = (position['price'] -
                            position['stop_loss']) * position['size']
                    total_risk += risk

            self.current_risk_exposure = total_risk / current_equity

            # 5. 更新风险预算使用情况
            self.risk_budget_used = self.current_risk_exposure / \
                self.get_param("max_risk_exposure")
        ...
        try:
            ...
            for existing_stk, position in self.positions.items():
                if existing_stk not in self.correlation_matrix:
                    # 计算相关性并缓存
                    existing_k = existing_stk.get_kdata(
                        Query(datetime - 30, datetime))
    ...
    def _get_sell_num(self, datetime, stk, price, risk, part_from):
        try:
            ...
            if part_from == System.Part.STOPLOSS:
                ...
                # 止盈可以分批卖出
                sell_ratio = self._calculate_sell_ratio(
                    datetime, stk, price, position)
