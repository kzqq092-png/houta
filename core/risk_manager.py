...
class RiskManager:
    ...
    def _load_risk_params(self):
        ...
        try:
            ...
            self.logger.error(f"加载风险控制参数失败: {str(e)}")
    ...
    def _check_position_limit(self, signal: Dict) -> bool:
        ...
        try:
            if signal['type'] == 'buy':
                # 计算当前总持仓比例
                total_position = sum(self.current_positions.values()) / self.current_equity
                
                # 检查是否超过最大持仓比例
                if total_position >= self.max_position_size:
                    self.logger.warning("超过最大持仓比例限制")
                    return False
                ...
                if position_ratio > self.max_single_position:
                    self.logger.warning("超过单个股票最大持仓比例限制")
                    return False
    ...
    def _check_stop_loss(self, signal: Dict) -> bool:
        ...
        try:
            ...
            if profit_ratio <= -self.stop_loss:
                self.logger.warning(f"触发止损: {stock_code}")
                return False
                
            return True
            
        except Exception as e:
            self.logger.error(f"检查止损失败: {str(e)}")
            return False
        ...
        try:
            ...
            if self.peak_equity > 0:
                ...
                if drawdown >= self.max_drawdown:
                    self.logger.warning("超过最大回撤限制")
                    return False
                    
            return True
            
        except Exception as e:
            self.logger.error(f"检查回撤失败: {str(e)}")
            return False
        ...
        try:
            ...
            if stock_code not in self.current_positions:
                ...
                if new_amount <= 0:
                    ...
                    # 更新持仓均价
                    if amount > 0:
                        total_cost = current['amount'] * current['avg_cost'] + amount * price
    ...
    def update_equity(self, equity: float):
        ...
        try:
            ...
            self.logger.error(f"更新权益信息失败: {str(e)}")
    ...
    def get_risk_metrics(self) -> Dict:
        ...
        try:
            ...
            position_ratio = total_position / self.current_equity if self.current_equity > 0 else 0
            ...
            self.logger.error(f"获取风险指标失败: {str(e)}")
            return {}
