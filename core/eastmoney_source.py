...
class EastMoneyDataSource(DataSource):
    ...
    def _calculate_sentiment_index(self, up_count, down_count, total):
        """计算市场情绪指数"""
        if total == 0:
            return 50.0
    ...
    def _calculate_turnover_rate(self, market_data):
        """计算换手率"""
        try:
            return sum(float(item["f8"]) for item in market_data["diff"]) / len(market_data["diff"])
        except:
            return 0.0
    ...
    def _calculate_fear_index(self, up_count, down_count, total):
        """计算恐慌指数"""
        if total == 0:
            return 50.0
        return (down_count / total) * 100.0
