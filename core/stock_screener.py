...
class StockScreener:
    ...
    def screen_stocks(self, 
        ...
        try:
            ...
            if strategy_type == "技术指标筛选":
                results = self.screen_by_technical(stock_list, technical_params)
            elif strategy_type == "基本面筛选":
                results = self.screen_by_fundamental(stock_list, fundamental_params)
    ...
    def screen_by_technical(self, 
        ...
        for stock in stock_list:
            try:
                ...
                # 计算技术指标
                ma_short = MA(kdata, params['ma_short'])
                ma_long = MA(kdata, params['ma_long'])
                macd = MACD(kdata)
                rsi = RSI(kdata)
                
                # 检查条件
                if self.check_ma_cross(ma_short, ma_long) and \
                   self.check_macd_signal(macd, params['macd_signal']) and \
                   self.check_rsi_condition(rsi, params['rsi_operator'], params['rsi_value']):
                    # 获取股票信息
    ...
    def screen_by_capital(self,
        ...
        for stock in stock_list:
            try:
                # 获取资金流向数据
                main_force = self.get_main_force(stock, params['main_force_days'])
    ...
    def screen_by_all(self,
        ...
        """
        # 分别进行筛选
        technical_results = self.screen_by_technical(stock_list, technical_params)
        fundamental_results = self.screen_by_fundamental(stock_list, fundamental_params)
        capital_results = self.screen_by_capital(stock_list, capital_params)
        
        # 合并结果
        results = pd.concat([technical_results, fundamental_results, capital_results])
    ...
    def save_strategy(self, strategy_type: str, conditions: dict, template_name: str = None):
        ...
        try:
            ...
            self.log_manager.log(f"保存策略模板失败: {str(e)}", "error")
    ...
    def load_strategy(self, template_name: str) -> tuple:
        ...
        """
        try:
            template_path = os.path.join("templates/stock_screener", f"{template_name}.json")
            ...
            self.log_manager.log(f"加载策略模板失败: {str(e)}", "error")
            return None, None
    ...
    def list_templates(self) -> list:
        ...
        try:
            ...
            self.log_manager.log(f"获取策略模板列表失败: {str(e)}", "error")
            return []
    ...
    def delete_template(self, template_name: str):
        ...
        try:
            ...
            self.log_manager.log(f"删除策略模板失败: {str(e)}", "error") 