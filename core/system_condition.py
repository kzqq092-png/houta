...
class EnhancedSystemCondition(ConditionBase):
    ...
    def __init__(self, params=None):
        super(EnhancedSystemCondition, self).__init__("EnhancedSystemCondition")
    ...
    def _calculate_indicators(self, k):
        ...
        try:
            ...
            # 计算成交量均线
            volume_ma = MA(VOL(k), n=self.get_param("volume_ma_period"))
    ...
    def _detect_market_regime(self, k, indicators):
        ...
        try:
            # 获取价格和均线
            close = CLOSE(k)
            ma = indicators.get('ma', MA(CLOSE(k), n=self.get_param("ma_period")))
    ...
    def _calculate_volatility(self, k):
        ...
        try:
            ...
            volatility = returns.rolling(window=self.get_param("volatility_period")).std()
            return volatility[-1]
        except Exception as e:
