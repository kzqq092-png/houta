...
class IndicatorCombinationManager:
    ...
    def __init__(self, data_dir: str = None):
        ...
        if data_dir is None:
            data_dir = os.path.join(
                os.getcwd(), 'config', 'indicator_combinations')
        ...
        logger.info(
            f"Indicator combination manager initialized with {len(self.combinations)} combinations")
        ...
        if self.combinations_file.exists():
            try:
                ...
                for name, combination_data in data.items():
                    combinations[name] = IndicatorCombination.from_dict(
                        combination_data)

                logger.debug(
                    f"Loaded {len(combinations)} indicator combinations")
    ...
    def _save_combinations(self) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Saved {len(self.combinations)} indicator combinations")
            return True
    ...
    def save_combination(self, name: str, indicators: List[Dict[str, Any]],
        ...
        try:
            ...
            if self._save_combinations():
                ...
                logger.info(
                    f"Indicator combination '{name}' {action} successfully")
                return True
            else:
                return False
    ...
    def load_combination(self, name: str) -> Optional[IndicatorCombination]:
        ...
        try:
            if name in self.combinations:
                combination = self.combinations[name]
                logger.debug(
                    f"Loaded combination '{name}' with {len(combination.indicators)} indicators")
                return combination
            else:
    ...
    def delete_combination(self, name: str) -> bool:
        ...
        try:
            if name in self.combinations:
                ...
                if self._save_combinations():
                    logger.info(
                        f"Indicator combination '{name}' deleted successfully")
                    return True
                else:
                    return False
            else:
    ...
    def search_combinations(self, query: str = "", tags: List[str] = None) -> Dict[str, IndicatorCombination]:
        ...
        for name, combination in self.combinations.items():
            ...
            # 标签匹配
            tag_match = not tags or any(
                tag in combination.tags for tag in tags)
        ...
        logger.debug(
            f"Found {len(results)} combinations matching query '{query}'")
        return results
            ...
            for indicator in combination.indicators:
                ...
                if indicator_name:
                    stats['most_used_indicators'][indicator_name] = \
                        stats['most_used_indicators'].get(
                            indicator_name, 0) + 1
    ...
    def export_combinations(self, file_path: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Exported {len(self.combinations)} combinations to {file_path}")
            return True
    ...
    def import_combinations(self, file_path: str, overwrite: bool = False) -> bool:
        ...
        try:
            ...
            if self._save_combinations():
                logger.info(
                    f"Imported {imported_count} combinations, skipped {skipped_count}")
                return True
            else:
                return False
