...
class MarketEnvironment(EnvironmentBase):
    ...
    def _calculate_indicators(self, k):
        ...
        try:
            ...
            # 计算移动平均线
            ma_short = MA(CLOSE(k), n=self.get_param("ma_short"))
            ma_mid = MA(CLOSE(k), n=self.get_param("ma_mid"))
            ma_long = MA(CLOSE(k), n=self.get_param("ma_long"))
            
            # 计算MACD
            macd = MACD(CLOSE(k), 
                       n1=self.get_param("macd_fast"),
                       n2=self.get_param("macd_slow"),
                       n3=self.get_param("macd_signal"))
            
            # 计算RSI
            rsi = RSI(CLOSE(k), n=self.get_param("rsi_period"))
            
            # 计算布林带
            boll = BOLL(CLOSE(k), 
                       n=self.get_param("boll_period"),
