...
# 统一数据库文件路径（已迁移到hikyuu_system.db）
...
class IndicatorService:
    ...
    def _get_best_implementation(self, indicator: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        # 首先尝试获取默认实现
        default_impl = next(
            (impl for impl in implementations if impl.get('is_default')), None)
        if default_impl:
            return default_impl
        ...
        if TALIB_AVAILABLE:
            talib_impl = next(
                (impl for impl in implementations if impl['engine'] == 'talib'), None)
            if talib_impl:
                return talib_impl

        # 如果没有TA-Lib或没有TA-Lib实现，尝试pandas实现
        pandas_impl = next(
            (impl for impl in implementations if impl['engine'] == 'pandas'), None)
        if pandas_impl:
            return pandas_impl
