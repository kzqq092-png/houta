...
class EnhancedTradingSignal(SignalBase):
        ...
        try:
            ...
            for feature_name in self.get_param("ml_features"):
                if feature_name == "close":
                    ...
                    features.append(RSI(CLOSE(k), n=self.get_param("rsi_window")))
                # 添加更多特征...
    ...
    def _calculate(self, k, record):
        try:
            ...
            # 2. 计算额外技术指标
            kdj = KDJ(k, n=self.get_param("kdj_n"))
            boll = BOLL(CLOSE(k), n=self.get_param("boll_n"), width=self.get_param("boll_width"))
