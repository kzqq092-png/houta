...
logger = logging.getLogger('update_ui_references')

# 需要检查的UI组件目录
UI_DIRS = [
    os.path.join(parent_dir, 'gui', 'widgets'),
    os.path.join(parent_dir, 'gui', 'dialogs'),
    os.path.join(parent_dir, 'gui', 'components')
]
...
def find_py_files(directories: List[str]) -> List[str]:
    """
    在指定目录中查找所有Python文件
...
def main():
    ...
    # 解析命令行参数
    import argparse
    parser = argparse.ArgumentParser(description='更新UI组件中的指标引用')
    parser.add_argument('--dry-run', action='store_true', help='模拟运行，不实际修改文件')
    parser.add_argument('--dirs', nargs='+', help='要检查的目录列表，默认为UI_DIRS')
    args = parser.parse_args()

    # 获取要检查的目录
    directories = args.dirs if args.dirs else UI_DIRS
    logger.info(f"将检查以下目录: {directories}")

    # 查找Python文件
    py_files = find_py_files(directories)
